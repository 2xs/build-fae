...
// address in `lr`.
//
// This ARM-state Rust runtime is intended to implement the same minimal ABI as
// `rt0/arm-arm/rt0-arm.s`, only in Rust.
//
...
#[unsafe(no_mangle)]
unsafe extern "C" fn __rustrt0_arm_run(ram_start: *mut u8, out: *mut Rt0Result) {
    let out = &mut *out;
    ...
    let metadata_addr = (&__metadataOff as *const u32) as usize;
    let startup_addr = _start as *const () as usize;
    let fae_base_bias = startup_addr.wrapping_sub(FAE_FOOTER_SIZE);
    ...
    let end_of_binary = fae_base_bias.wrapping_add(binary_size);
    ...
    let rel_got_sec_addr = ram_start as usize;
