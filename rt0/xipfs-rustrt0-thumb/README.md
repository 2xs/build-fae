...
`xipfs-rustrt0-thumb` is a Rust implementation of the XiPFS-compatible CRT0 startup logic for ARM Thumb targets.
...
```bash
cargo run -p xtask -- xipfs-rustrt0-thumb
```

The generated binary is:

- `build/xipfs-rustrt0-thumb.bin`

The corresponding ELF with symbols is kept under:

- `target/thumbv7em-none-eabi/release/xipfs-rustrt0-thumb`
